...
export const deleteHandler = (params: {
  ...
  const { erxesApiUrl } = params;

  const {
    url = REACT_APP_API_URL !== undefined
      ? `${REACT_APP_API_URL}/upload-file`
      : `${erxesApiUrl}/upload-file`,
    fileName,
    afterUpload
  } = params;
...
const uploadHandler = (params: Params) => {
  ...
  console.log('erxesApiUrl');
  console.log(erxesApiUrl);

  console.log('erxes-ui url');
  console.log(url);
