...
const colorTypes = (type, value) => {
  switch (type) {
    ...
    case 'danger':
      ...
      if (value === 'background')
        return rgba(colors.colorCoreRed, 0.1);
      if (value === 'icon')
        return 'times-circle';
    case 'info':
      if (value === 'content')
    ...
    default:
      ...
      if (value === 'background')
        return rgba(colors.colorPrimary, 0.1);
      if (value === 'icon')
        return 'envelope-alt';
  }
};

const InfoBox = styledTS<{ color?: string; type?: string, iconShow }>(styled.div)`
  min-height: ${dimensions.coreSpacing * 2}px;
  padding: ${props => props.iconShow ? dimensions.unitSpacing / 2 : dimensions.unitSpacing + 5}px
    ${dimensions.coreSpacing}px;
  margin-bottom: ${dimensions.coreSpacing}px;
  background-color: ${props => colorTypes(props.type, 'background')};
            
  border: ${props => `${addition / 2}px solid ${colorTypes(props.type, 'background')}`};
...
class Info extends React.PureComponent<Props> {
  ...
  render() {
    const { title, children, color, type = 'primary', iconShow = false } = this.props;
